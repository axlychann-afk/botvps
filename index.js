...
const PRICE = Number(process.env.PRICE || 2000);
const UNIT_PRICE = Math.round(PRICE / 2); // 1k/unit, 2 unit per order
...
async function buildStart(name, chatId) {
  ...
  const buttons = empty
    ...
    : Markup.inlineKeyboard([
        ...
        [Markup.button.callback('💳 Bayar Pakai Saldo', 'buy_balance'), Markup.button.callback('➕ Isi Saldo', 'topup')],
      ]);
  return { text, buttons: { ...buttons, parse_mode: 'HTML' } };
}
