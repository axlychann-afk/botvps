...
// Error TIDAK di-retry di sini (biar ga double-order ke API); disebar ke pemanggil.
let _q = Promise.resolve();
function queued(fn) {
  const run = () => fn().finally(() => new Promise((s) => setTimeout(s, 2200)));
  const next = _q.then(run, run);
  _q = next.catch(() => {});
  return next;
}
...
async function get(path, ms = 25000) {
  return queued(async () => {
    ...
    try {
      ...
      if (e?.name === 'AbortError') throw new Error(`API nomor timeout ${path}`);
      throw e;
    } finally {
      clearTimeout(t);
    }
...
export const NOKOS_CATALOG = [
  ...
  { serviceId: 5, label: 'Telegram', emoji: '✈️' },
];
